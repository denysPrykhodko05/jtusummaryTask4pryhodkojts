            ...
            <div>
                <c:choose>
                    <c:when test="${sessionScope.loginBool == true}">
                        <a href="<c:url value='/profile'/>"><p style="color: #291810;"><fmt:message key="index.profile"/></a></p><br>
                        <p style="color: #291810;"><a href="/logout"><fmt:message key="index.Logout"/> </a></p>
                    </c:when>
                    <c:otherwise>
                        <a href="/login"><p style=" color: #291810;;"><fmt:message key="index.login"/></p></a>
                    </c:otherwise>
                </c:choose>
                <label for="from">
    ...
    <table border="1">
        ...
        <c:forEach var="route" items="${routes}">
            <tr>
                ...
                <td>
                    <c:if test="${route.economyClass>0}">
                ...
                <td>
                    <c:if test="${route.economyClass>0}">
                        <form method="get" action="<c:url value='/purchaseTicket'/>">
                            ...
                            <fmt:message key="index.economyClass"/>: ${route.economyClass * 58} <input
                                type="submit"
                                name="economyClass"
                    ...
                    <c:if test="${route.common>0}">
                        <form method="get" action="<c:url value='/purchaseTicket'/>">
                            ...
                            <input type="hidden" id="arrive_time" name="arrive_time" value="${route.arrive_time}">
                            <input type="hidden" id="depart_time" name="depart_time" value="${route.depart_time}">
                            <fmt:message key="index.common"/> : ${route.common * 68} <input type="submit"
                                                                                            name="common"
                                                                                            value="<fmt:message key="index.choose"/>"><br>
                        </form>
                    </c:if>
